...
impl Launch {
    pub(crate) fn launch(mut self) {
        println!(">-tokio/tokio/src/runtime/thread_pool/worker.rs>>Launch::launch()");
...
fn run(worker: Arc<Worker>) {
    println!(">*tokio/tokio/src/runtime/thread_pool/worker.rs>>run()");
...
impl Context {
    fn run(&self, mut core: Box<Core>) -> RunResult {
        println!(">*tokio/tokio/src/runtime/thread_pool/worker.rs>>Context::run()");
        while !core.is_shutdown {
            println!("\t\t>*tokio/tokio/src/runtim/thread_pool/worker.rs>>Context::run() is looping");
            // Increment the tick
            core.tick();
            ...
            if let Some(task) = core.next_task(&self.worker) {
                println!("\t\t>*tokio/tokio/src/runtime/thread_pool/worker.rs>>Context::run() core.next_task is true");
                core = self.run_task(task, core)?;
                continue;
            }

            println!("\t\t>*tokio/tokio/src/runtime/thread_pool/worker.rs>>Context::run() try steal work!");
        ...
        println!("\x1b[91m>*tokio/tokio/src/rutime/thread_pool/worker.rs>>Context::run()\x1b[0m");
        Err(())
    }

    fn run_task(&self, task: Notified, mut core: Box<Core>) -> RunResult {
        println!(">*tokio/tokio/src/runtime/thread_pool/worker.rs>>Context::run_task()");
    ...
    fn park(&self, mut core: Box<Core>) -> Box<Core> {
        println!(">*tokio/tokio/src/runtime/thread_pool/worker.rs>>Context::park()");
        ...
        println!("\x1b[91m>*tokio/tokio/src/runtime/thread_pool/worker.rs>>Context::park()\x1b[0m");
        core
    }

    fn park_timeout(&self, mut core: Box<Core>, duration: Option<Duration>) -> Box<Core> {
        println!(">*tokio/tokio/src/runtime/thread_pool/worker.rs>>Context::park_timeout()");
...
impl Core {
    ...
    fn next_task(&mut self, worker: &Worker) -> Option<Notified> {
        println!(">*tokio/tokio/src/runtime/thread_pool/worker.rs>>Core::next_task()<");
    ...
    fn next_local_task(&mut self) -> Option<Notified> {
        println!(">*tokio/tokio/src/runtime/thread_pool/worker.rs>>Core::next_local_task()<");
    ...
    fn steal_work(&mut self, worker: &Worker) -> Option<Notified> {
        println!(">*tokio/tokio/src/runtime/thread_pool/worker.rs>>Core::steal_work()<");
...
impl Worker {
    ...
    fn inject(&self) -> &Inject<Arc<Shared>> {
        println!(">*tokio/tokio/src/runtime/thread_pool/worker.rs>>Worker::inject()");
        &self.shared.inject
    }
}
...
impl Shared {
    ...
    {
        println!(">#tokio/tokio/src/runtime/thread_pool/worker.rs>>Shared::bind_new_task()");
        let (handle, notified) = me.owned.bind(future, me.clone());

        if let Some(notified) = notified {
            me.schedule(notified, false); // traced
        }
        println!("\x1b[91m>#tokio/tokio/src/runtime/thread_pool/worker.rs>>Shared::bind_new_task()\x1b[0m");
        handle
    }

    pub(super) fn schedule(&self, task: Notified, is_yield: bool) {
        println!(">#tokio/tokio/src/runtime/thread_pool/worker.rs>>Shared::schedule()<");
        CURRENT.with(|maybe_cx| {
            if let Some(cx) = maybe_cx {
    ...
    fn schedule_local(&self, core: &mut Core, task: Notified, is_yield: bool) {
        println!(">tokio/tokio/src/runtime/thread_pool/worker.rs>>Shared::schedule_local()");
