...
cfg_rt! {
    ...
    impl Runtime {
        ...
        {
			println!(">tokio/tokoi/src/runtime/mod.rs>>Runtime::spawn()<");
        ...
        pub fn block_on<F: Future>(&self, future: F) -> F::Output {
			println!(">+tokio/tokio/src/runtime/mod.rs>>Runtime::block_on()");
            #[cfg(all(tokio_unstable, feature = "tracing"))]
            let future = crate::util::trace::task(future, "block_on", None);

            let _enter = self.enter(); //traced

            let temp = match &self.kind { // traced
                Kind::CurrentThread(exec) => exec.block_on(future),
                #[cfg(feature = "rt-multi-thread")]
                Kind::ThreadPool(exec) => exec.block_on(future),
            };
			println!("\x1b[91m>+tokio/tokio/src/runtime/mod.rs>>Runtime::block_on()\x1b[0m");
			temp
        }
        ...
        pub fn enter(&self) -> EnterGuard<'_> {
            println!(">+tokio/tokio/src/runtime/mod.rs>>Runtime::enter()<");
            self.handle.enter()
        }
