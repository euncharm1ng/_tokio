...
cfg_rt! {
    ...
    {
		println!(">#tokio/tokio/src/task/spawn.rs>>spawn()<");
    ...
    where
        ...
        T::Output: Send + 'static,
    {
        println!(">#tokio/tokio/src/task/spawn.rs>>spawn_inner()<");
        let spawn_handle = crate::runtime::context::spawn_handle().expect(CONTEXT_MISSING_ERROR); // traced
        let task = crate::util::trace::task(future, "task", name); // traced
        spawn_handle.spawn(task) // traced
    }
}
